...
import {deleteDictionary, selectDictionary} from "@/services/contract/common/dictionary";

/* React.FC<>的在typescript使用的一个泛型，FC就是FunctionComponent的缩写，是函数组件，在这个泛型里面可以使用useState */
const Applications: React.FC = () => {
  const [isModalVisible, setIsModalVisible] = useState(false);
  const [dictionaryId, setDictionaryId] = useState(undefined);
    ...
    <PageContainer title={false} ghost>
      <ProTable
        ...
        request={async (params) => {   //调用请求加载表格数据， 默认自动加载 params为Search的查询条件参数
          ...
          return await selectDictionary({...newParams});
        }}
      />
